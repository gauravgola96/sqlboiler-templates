...
func (o *{{$alias.UpSingular}}) Insert({{if .NoContext}}exec boil.Executor{{else}}ctx context.Context, exec boil.ContextExecutor{{end}}, columns boil.Columns) error {
	...
	if database_==nil{
		return errors.New("Empty database in context")
	}
	...
	key := makeCacheKey(columns, database, nzDefaults)
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"%s\".{{$schemaTable}} ({{.LQ}}%s{{.RQ}}) %%sVALUES (%s)%%s", database, strings.Join(wl, "{{.RQ}},{{.LQ}}"), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			{{if .Dialect.UseDefaultKeyword -}}
			cache.query = fmt.Sprintf("INSERT INTO \"%s\".{{$schemaTable}} %sDEFAULT VALUES%s", database)
			{{else -}}
			cache.query = fmt.Sprintf("INSERT INTO \"%s\".{{$schemaTable}} () VALUES ()%s%s", database)
			{{end -}}
		}
		...
		if len(cache.retMapping) != 0 {
			...
			cache.retQuery = fmt.Sprintf("SELECT {{.LQ}}%s{{.RQ}} FROM \"%s\".{{$schemaTable}} WHERE %s", strings.Join(returnColumns, "{{.RQ}},{{.LQ}}"), database,strmangle.WhereClause("{{.LQ}}", "{{.RQ}}", {{if .Dialect.UseIndexPlaceholders}}1{{else}}0{{end}}, {{$alias.DownSingular}}PrimaryKeyColumns))
			{{else -}}
