...
func {{$alias.UpSingular}}Exists({{if .NoContext}}exec boil.Executor{{else}}ctx context.Context, exec boil.ContextExecutor{{end}}, {{$pkArgs}}) (bool, error) {
	...
	if database_==nil{
		return false, errors.New("Empty database in context")
	}
	database := fmt.Sprintf("%v", database_)
	
	var exists bool
	{{if .Dialect.UseCaseWhenExistsClause -}}
	sql := fmt.Sprintf("select case when exists(select top(1) 1 from \"%s\".{{$schemaTable}} where {{if .Dialect.UseIndexPlaceholders}}{{whereClause .LQ .RQ 1 .Table.PKey.Columns}}{{else}}{{whereClause .LQ .RQ 0 .Table.PKey.Columns}}{{end}}) then 1 else 0 end", database)
	{{- else -}}
	sql := fmt.Sprintf("select exists(select 1 from \"%s\".{{$schemaTable}} where {{if .Dialect.UseIndexPlaceholders}}{{whereClause .LQ .RQ 1 .Table.PKey.Columns}}{{else}}{{whereClause .LQ .RQ 0 .Table.PKey.Columns}}{{end}}{{if and .AddSoftDeletes $canSoftDelete}} and {{or $.AutoColumns.Deleted "deleted_at" | $.Quotes}} is null{{end}} limit 1)", database)
	{{- end}}
