...
func (o *{{$alias.UpSingular}}) Delete({{if .NoContext}}exec boil.Executor{{else}}ctx context.Context, exec boil.ContextExecutor{{end}}{{if $soft}}, hardDelete bool{{end}}) {{if .NoRowsAffected}}error{{else}}(int64, error){{end -}} {
	database_ := ctx.Value("database")
	if database_==nil{
		return 0, errors.New("Empty database in context")
	}
	database := fmt.Sprintf("%v", database_)
	...
	if hardDelete {
		...
		sql = fmt.Sprintf("DELETE FROM \"%s\".{{$schemaTable}} WHERE {{if .Dialect.UseIndexPlaceholders}}{{whereClause .LQ .RQ 1 .Table.PKey.Columns}}{{else}}{{whereClause .LQ .RQ 0 .Table.PKey.Columns}}{{end}}", database)
	} else {
		currTime := time.Now().In(boil.GetLocation())
		o.{{$alias.Column $softDelCol}} = null.TimeFrom(currTime)
		wl := []string{"{{$softDelCol}}"}
		sql = fmt.Sprintf("UPDATE \"%s\".{{$schemaTable}} SET %s WHERE {{if .Dialect.UseIndexPlaceholders}}{{whereClause .LQ .RQ 2 .Table.PKey.Columns}}{{else}}{{whereClause .LQ .RQ 0 .Table.PKey.Columns}}{{end}}",
			database, strmangle.SetParamNames("{{.LQ}}", "{{.RQ}}", {{if .Dialect.UseIndexPlaceholders}}1{{else}}0{{end}}, wl),
		)
...
func (q {{$alias.DownSingular}}Query) DeleteAll({{if .NoContext}}exec boil.Executor{{else}}ctx context.Context, exec boil.ContextExecutor{{end}}{{if $soft}}, hardDelete bool{{end}}) {{if .NoRowsAffected}}error{{else}}(int64, error){{end -}} {
	...
	queries.SetFrom(q.Query , database+".{{.Table.Name}}")
	{{if $soft -}}
	if hardDelete {
...
func (o {{$alias.UpSingular}}Slice) DeleteAll({{if .NoContext}}exec boil.Executor{{else}}ctx context.Context, exec boil.ContextExecutor{{end}}{{if $soft}}, hardDelete bool{{end}}) {{if .NoRowsAffected}}error{{else}}(int64, error){{end -}} {
	...
	if hardDelete {
		...
		sql = fmt.Sprintf("DELETE FROM \"%s\".{{$schemaTable}} WHERE ", database) +
		...
		wl := []string{"{{$softDelCol}}"}
		sql = fmt.Sprintf("UPDATE \"%s\".{{$schemaTable}} SET %s WHERE " +
			...
			databsae, strmangle.SetParamNames("{{.LQ}}", "{{.RQ}}", {{if .Dialect.UseIndexPlaceholders}}1{{else}}0{{end}}, wl),
		)
